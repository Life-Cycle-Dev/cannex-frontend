...
export default function NewsroomCarousel({ items }: { items: Item[] }) {
  ...
  if (!items || items.length === 0) {
    return (
      <div className="flex justify-between h-[315px] tablet:h-[415px] border-b-2">
  ...
  return (
    <div className="flex justify-between h-[315px] tablet:h-[415px] tablet:border-b-2">
      <div className="min-w-[79px] h-full border-r-2 z-1 hidden tablet:block"></div>
      ...
      <div key={current.id} className="w-full flex tablet:flex-row flex-col">
        ...
        />
        <div className="relative p-4 w-full border-b-2 tablet:border-0">
          <div className="absolute top-4 right-4">
            <RightUpIcon />
          </div>

          <div className="p-4 pt-10 pb-16 tablet:p-16 flex flex-col gap-3">
            <div className="font-bold text-5xl/tight">
              {current?.title ?? "-"}
            </div>
          ...
          <div className="absolute bottom-7 left-8 tablet:left-20 flex gap-2">
            {items.map((_, i) => (
              <button
                key={i}
          ...
          <div className="absolute bottom-0 right-0 flex">
            ...
            >
              <RightUpIcon
                className={`-rotate-135 w-8 h-8 ${
                  index === 0 && "text-neutral100"
                }`}
              />
            </button>
            <button
            ...
            >
              <RightUpIcon
                className={`rotate-45 w-8 h-8 ${
                  index === items.length - 1 && "text-neutral100"
                }`}
              />
            </button>
          </div>
      ...
      <div className="min-w-[79px] h-full border-l-2 hidden tablet:block"></div>
    </div>
  );
}
